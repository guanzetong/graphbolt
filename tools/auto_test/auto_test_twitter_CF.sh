...
do
    ...
    # then
        ...
        # do
            ...
            ./CF -ae -nWorkers $CORE_NUM -numberOfUpdateBatches 10 -nEdges $BATCH_SIZE -maxIters 100 -partitionsFile $PARTITION_FILE -streamPath $STREAM_PATH $INIT_GRAPH > $PROGRAM_OUT
            
            rm -f $SNAPSHOT_OUT
            for BATCH_NUM in {1..10..1}
            do
                echo "Running Snapshot No.$BATCH_NUM ..."
                ./CF -ae -nWorkers $CORE_NUM -numberOfUpdateBatches 0 -nEdges 0 -maxIters 100 -partitionsFile $PARTITION_FILE -streamPath ${EMPTY_STREAM_PATH} ${SNAPSHOT_PATH}_$BATCH_NUM.adj >> $SNAPSHOT_OUT
            done
